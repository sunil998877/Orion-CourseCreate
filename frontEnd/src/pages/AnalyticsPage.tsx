...
const AnalyticsPage: React.FC = () => {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (<div className="bg-gray-800 border border-gray-700 p-2 rounded shadow text-white text-xs">
    ...
    return (<PageTransition>
      <div className="min-h-screen text-white relative font-sans selection:bg-lime-500/30">
        <div className="space-y-10 pb-20">
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
              ...
              <div className="flex items-center gap-8 md:gap-12 relative max-md:w-full max-md:flex-col-reverse max-md:gap-2">
                ...
                <div className="relative z-50 max-md:w-full">
                  <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center gap-3 bg-black/40 border border-white/10 rounded-xl px-5 py-3 text-sm font-medium text-white hover:bg-white/5 hover:border-lime-500/30 hover:shadow-[0_0_15px_rgba(132,204,22,0.1)] transition-all duration-300 group/btn max-md:w-full max-md:justify-between">
                  ...
                  {dropdownOpen && (<>
                      ...
                      <div className="absolute right-0 top-full mt-3 w-56 max-md:left-0 max-md:right-0 max-md:w-full max-md:top-auto max-md:bottom-full max-md:mt-0 max-md:mb-3 bg-[#0f1115] border border-white/10 rounded-xl shadow-2xl z-[60] overflow-hidden ring-1 ring-black/50 animate-in fade-in zoom-in-95 duration-200">
                        {rangeOptions.map(option => (<button key={option.value} onClick={() => {
                ...
                <div className="relative w-56 h-72 md:w-64 md:h-80 flex items-end justify-center pointer-events-auto select-none perspective-[1200px] group/holo max-md:w-36 max-md:h-48 max-md:mx-auto max-md:shrink-0">
                  ...
                  <div className="absolute inset-0 overflow-hidden rounded-full mask-image-[radial-gradient(circle,black,transparent)] pointer-events-none">
                    {[...Array(6)].map((_, i) => (<div key={i} className="absolute bottom-0 text-[10px] font-mono text-lime-400/60 animate-up-fade" style={{
                ...
                animationDelay: `${Math.random() * 2}s`
                  ...
                  <div className="relative z-10 w-full h-full flex items-end justify-center">
                    ...
                    <div className="relative w-full h-full transition-transform duration-500 group-hover/holo:scale-[1.02]">
                      <img src={avtar3} alt="AI Analyst" className="h-full w-auto object-contain object-bottom
                      ...
                      <img src={avtar3} alt="" className="absolute inset-0 h-full w-auto object-contain object-bottom opacity-40 blur-[2px] mix-blend-screen animate-pulse"/>
                    </div>
                  </div>
        ...
        ].map((stat, idx) => (<div key={idx} className="group relative bg-gray-900/40 border border-white/5 rounded-2xl p-6 backdrop-blur-md hover:bg-white/5 hover:border-lime-500/20 transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_10px_30px_-10px_rgba(132,204,22,0.1)]">
          ...
          <div className="space-y-6">
            ...
            <div className="relative rounded-[2rem] bg-gray-900/40 border border-white/5 backdrop-blur-xl p-8 shadow-2xl h-[500px] hover:border-white/10 transition-colors duration-700 max-md:h-[280px] max-md:rounded-2xl max-md:p-3">
              ...
              {loading ? (<div className="h-full flex flex-col items-center justify-center text-white/30 space-y-4">
                ...
                </div>) : chartReady ? (<ResponsiveContainer width="100%" height="100%" minWidth={0} debounce={50}>
                  <BarChart data={buckets} margin={{ top: 20, right: 0, left: -20, bottom: 40 }}>
                    ...
                    <XAxis dataKey="label" stroke="#ffffff40" tickFormatter={formatXAxis} fontSize={11} tickLine={false} axisLine={false} dy={20}/>
                    <YAxis stroke="#ffffff40" fontSize={11} tickLine={false} axisLine={false} allowDecimals={false}/>
                    <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(132,204,22,0.05)', radius: 8 }}/>
                    <Bar dataKey="count" fill="url(#barGradient)" radius={[6, 6, 6, 6]} barSize={40} animationDuration={1500}/>
                  </BarChart>
