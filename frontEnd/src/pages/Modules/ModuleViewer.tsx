...
interface ModuleViewerProps {
    ...
    onClose: () => void;
    onRegenerate?: () => void;
    onRefine?: (prompt: string, history: {
        role: 'user' | 'assistant';
        content: string;
    }[]) => Promise<string>;
...
export const ModuleViewer: React.FC<ModuleViewerProps> = ({ moduleData, onClose, onRegenerate, onRefine, isRegenerating, refineProgress, credit, duration }) => {
    ...
    return createPortal(<div className="fixed inset-0 flex items-center justify-center z-[9999] px-4 py-4 animate-in fade-in duration-300 bg-gray-950/40 backdrop-blur-xl">
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] flex flex-col bg-gray-900/90 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 overflow-hidden text-white z-10 animate-in zoom-in-95 duration-300 backdrop-blur-2xl">
        ...
        <div className="relative px-8 py-10 shrink-0 border-b border-white/5 bg-white/[0.02] max-md:px-4 max-md:py-5">
          <div className="relative flex items-center justify-between max-md:flex-col max-md:items-stretch max-md:gap-4">
            ...
            <button onClick={onClose} className="group relative px-6 py-3 rounded-2xl bg-white/5 border border-white/10 hover:border-lime-500/30 hover:bg-lime-500/5 transition-all duration-300 flex items-center gap-2 animate-in slide-in-from-right duration-500 max-md:w-full max-md:justify-center">
        ...
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar bg-transparent max-md:p-4">
          {!moduleData || !moduleData.Content ? (<div className="flex flex-col items-center justify-center py-20 animate-in fade-in duration-700">
            ...
            </div>) : (<div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
              ...
              <div className="grid grid-cols-1 gap-12">
                <section>
                  ...
                  <div className="bg-white/[0.02] border border-white/5 p-8 rounded-3xl space-y-4">
                    {moduleData.Content.Objectives?.map((obj: string, i: number) => (<div key={i} className="flex items-start gap-4 group">
                ...
                <section>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {moduleData.Content.TeachingContent?.map((item: any, i: number) => (<div key={i} className="group bg-white/[0.02] hover:bg-white/[0.05] p-6 rounded-3xl border border-white/5 hover:border-lime-500/20 transition-all duration-300">
                        ...
                        <div className="space-y-3">
                          {item.ContentPoints?.slice(0, 3).map((pt: string, j: number) => (<div key={j} className="flex items-start gap-3">
                ...
                <section>
                  ...
                  <div className="bg-white/[0.02] border border-white/5 rounded-3xl overflow-hidden">
                    {moduleData.Content.TeachingContent?.map((item: any, i: number) => (<div key={i} className="flex justify-between items-center px-8 py-4 border-b border-white/5 last:border-0 hover:bg-white/[0.03] transition-colors max-md:flex-col max-md:items-start max-md:gap-2 max-md:px-4">
                ...
                <section>
                  ...
                  <div className="space-y-6">
                    {moduleData.Content.Quizzes?.map((quiz: any, i: number) => (<div key={i}>
                        {quiz.QuizDescription && (<p className="text-sm text-gray-500 font-medium mb-4 italic">{quiz.QuizDescription}</p>)}
                        <div className="space-y-4">
                          {quiz.Questions?.map((q: string, j: number) => (<div key={j} className="bg-white/[0.03] p-6 rounded-2xl border border-white/5 hover:border-white/10 transition-all">
              ...
              {onRefine && (<div className="mt-20 pt-16 border-t border-white/5 relative">
                  ...
                  <div className="max-w-2xl mx-auto space-y-8">
                    ...
                    {messages.length > 0 && (<div className="space-y-6 max-h-[400px] overflow-y-auto px-4 custom-scrollbar">
                        ...
                        role: 'user' | 'assistant';
                        content: string;
                    }, i: number) => (<div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in slide-in-from-bottom-2 duration-300`}>
                            <div className={`max-w-[85%] p-5 rounded-2xl text-sm font-medium leading-relaxed ${msg.role === 'user'
                        ...
                        {isThinking && (<div className="flex justify-start animate-in fade-in duration-300">
                    ...
                    <div className="relative group">
                      ...
                      <textarea value={customPrompt} onChange={(e) => setCustomPrompt(e.target.value)} placeholder="Make this module more technical, focus on regulatory compliance, or add an advanced case study..." className="relative w-full bg-white/[0.03] border border-white/10 rounded-2xl p-6 text-sm text-white focus:outline-none focus:ring-2 focus:ring-lime-500/30 transition-all placeholder:text-gray-600 resize-none h-32 font-medium" disabled={isThinking}/>
                    </div>

                    <div className="flex flex-col items-center gap-6 pb-10">
                      {isRegenerating && refineProgress !== undefined && (<div className="w-full max-w-md space-y-3 animate-in fade-in zoom-in-95 duration-500">
                          ...
                          <div className="w-full bg-white/5 rounded-full h-3 overflow-hidden border border-white/5 p-1">
                            <div className="bg-gradient-to-r from-lime-500 via-emerald-500 to-teal-500 h-full rounded-full transition-all duration-500 ease-out shadow-[0_0_20px_rgba(132,204,22,0.3)]" style={{ width: `${refineProgress}%` }}/>
                          </div>
                        </div>)}
                ...
                }} disabled={isRegenerating || isThinking} className="group relative flex items-center gap-4 bg-white text-black px-12 py-5 rounded-2xl font-black text-xl transition-all shadow-[0_20px_40px_rgba(0,0,0,0.3)] hover:shadow-[0_20px_40px_rgba(163,230,53,0.2)] hover:bg-lime-400 hover:-translate-y-1 active:translate-y-0 disabled:opacity-50 disabled:cursor-wait overflow-hidden max-md:w-full max-md:justify-center max-md:px-6 max-md:py-4 max-md:text-base">
