...
export const SlideContent: React.FC<SlideContentProps> = ({ moduleData, onClose }) => {
    ...
    return createPortal(
        <div className="fixed inset-0 flex items-center justify-center z-[9999] px-4 py-4 max-md:px-2 max-md:py-2 animate-in fade-in duration-300 bg-gray-950/40 backdrop-blur-xl">
            <div
            ...
            <div className={`relative w-full ${moduleData.showOrion ? 'max-w-7xl' : 'max-w-4xl'} max-h-[96vh] max-md:max-h-[100dvh] max-md:h-full flex flex-col bg-gray-900/90 rounded-3xl max-md:rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-white/10 overflow-hidden text-white z-10 transition-all duration-500 animate-in zoom-in-95 fade-in duration-300 backdrop-blur-2xl`}>
                ...
                <div className="relative px-8 py-10 shrink-0 border-b border-white/5 bg-white/[0.02] max-md:px-3 max-md:py-3">
                    <div className="relative flex items-center justify-between max-md:gap-2">
                        ...
                        <button
                            onClick={onClose}
                            className="group relative px-6 py-3 rounded-2xl bg-white/5 border border-white/10 hover:border-lime-500/30 hover:bg-lime-500/5 transition-all duration-300 flex items-center gap-2 animate-in slide-in-from-right duration-500 shrink-0 max-md:px-2.5 max-md:py-2.5 max-md:rounded-xl"
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="relative w-full h-[75vh] max-md:h-[calc(100dvh-8.5rem)] max-md:min-h-[320px] rounded-[2rem] max-md:rounded-xl overflow-hidden max-md:overflow-auto border border-white/10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.7)] group bg-black/40 backdrop-blur-sm">
                                        ...
                                        <iframe
                                            ...
                                            className="h-full w-full border-0 md:absolute md:inset-0 max-md:min-w-[640px] max-md:min-h-full"
                                            allowFullScreen
                                    ...
                                    <div className="grid grid-cols-1 gap-8">
                                        {Array.isArray(moduleData.slide?.Slides) && moduleData.slide?.Slides.length ? (
                                            moduleData.slide?.Slides?.map((slide: Record<string, unknown>, i: number) => {
                                                ...
                                                return (
                                                    <div key={i} className="group bg-white/[0.02] hover:bg-white/[0.04] p-8 rounded-[2rem] border border-white/5 hover:border-lime-500/20 transition-all duration-300 max-md:p-4 max-md:rounded-2xl">
