            ...
            <div className="relative z-10 flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
              <div className="space-y-4">
                ...
                {loading ? (<div className="flex gap-3">
                    ...
                    {credits.plan && (<span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-white/70">
                        <Sparkles className="h-3 w-3 text-lime-400"/>
                        {credits.plan} Plan
                      </span>)}
                    {credits.renewsOn && (<span className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs font-semibold text-white/70">
              ...
              <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-black/30 p-6 backdrop-blur-md max-md:max-w-none max-md:p-4">
                ...
                {loading ? (<div className="mt-2 space-y-3">
                ...
                : Math.round(usagePercentage)}% used
                    ...
                    {credits.reserved !== undefined && credits.reserved > 0 && (<p className="mt-2 text-[10px] text-yellow-400/70">
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
            <section className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-6 md:p-8 max-md:rounded-2xl max-md:p-4">
              {loading ? (<div className="space-y-3">
                ...
                </div>) : (<UsageHistory items={transactions} maxHeight="max-h-[420px]"/>)}
            </section>
            ...
            <section className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-6 md:p-8 max-md:rounded-2xl max-md:p-4">
              ...
              <div className="space-y-2">
                {Object.entries(CREDIT_COSTS).map(([action, cost]) => (<div key={action} className="flex items-center justify-between rounded-lg border border-white/5 bg-white/[0.02] px-4 py-3">
