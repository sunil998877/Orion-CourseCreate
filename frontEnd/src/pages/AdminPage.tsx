    ...
    return (<div className="space-y-8 transition-colors duration-200">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03] sm:p-8 backdrop-blur-md">
        <div className="relative z-10 flex flex-col justify-between gap-6 md:flex-row md:items-center">
          ...
          <div className="flex flex-wrap items-center gap-2.5 shrink-0">
            <button onClick={fetchStats} disabled={loading} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 cursor-pointer">
              <RefreshCw className={cn("h-3.5 w-3.5", loading && "animate-spin")}/>
              Refresh
            </button>
            <Link to="/admin/pricing" className="inline-flex items-center gap-2 rounded-xl border border-lime-500/30 bg-lime-500/10 px-4 py-2.5 text-xs font-semibold text-lime-700 hover:bg-lime-500/20 dark:border-lime-400/30 dark:bg-lime-400/10 dark:text-lime-400">
              <Coins className="h-4 w-4"/>
              Manage Pricing SKUs
            </Link>
            <Link to="/admin/users" className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-100 px-4 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-200 dark:border-white/10 dark:bg-white/5 dark:text-white/90 dark:hover:bg-white/10">
      ...
      <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {metricCards.map((card) => {
            ...
            return (<div key={card.title} className="group relative overflow-hidden rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-lime-500/40 hover:shadow-md dark:border-white/10 dark:bg-white/[0.03] dark:hover:border-lime-400/40">
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {providerWorkload.map((p) => {
            ...
            return (<div key={p.provider} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03]">
        ...
        <div className="mt-5 overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {recentTransactions.length > 0 ? (recentTransactions.map((tx: any) => (<tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
