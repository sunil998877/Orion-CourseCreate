...
export default function AdminSettingsPage() {
    ...
    return (<div className="max-w-4xl space-y-8 transition-colors duration-200">
      <div>
      ...
      {saved && (<div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-4 py-3 text-xs font-semibold text-emerald-700 dark:text-emerald-400 animate-in fade-in">
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03] space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <button type="button" onClick={() => setTheme('light')} className={cn("flex items-center gap-3 rounded-xl border p-4 text-left transition-colors cursor-pointer", !isDark
            ...
            <button type="button" onClick={() => setTheme('dark')} className={cn("flex items-center gap-3 rounded-xl border p-4 text-left transition-colors cursor-pointer", isDark
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03] space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="number" value={exchangeRate} onChange={(e) => setExchangeRate(Number(e.target.value))} className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-mono text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white"/>
            ...
            <div>
              ...
              <input type="number" value={minRecharge} onChange={(e) => setMinRecharge(Number(e.target.value))} className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-mono text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white"/>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-white/[0.03] space-y-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div>
              ...
              <input type="number" defaultValue={15} className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-mono text-slate-900 dark:border-white/10 dark:bg-white/5 dark:text-white"/>
            ...
            <div>
              ...
              <input type="text" defaultValue="Every 5 Minutes" disabled className="h-10 w-full rounded-xl border border-slate-200 bg-slate-100 px-3 text-xs text-slate-500 dark:border-white/10 dark:bg-white/[0.02]"/>
            </div>
          </div>
        </div>

        <button type="submit" className="inline-flex items-center gap-2 rounded-xl border border-lime-500/30 bg-lime-500/10 px-5 py-2.5 text-xs font-bold text-lime-700 hover:bg-lime-500/20 dark:border-lime-400/30 dark:bg-lime-400/10 dark:text-lime-400 cursor-pointer">
