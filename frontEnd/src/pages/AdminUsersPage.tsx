...
import { Users, Search, Plus, Minus, Check, X, Zap, Layers, FileSpreadsheet, Mic, BarChart3, Sparkles, Eye, RefreshCw, BookOpen, ExternalLink, ChevronDown, ChevronUp, CreditCard, Calendar, CheckCircle2, Clock, FileText, Headphones, ShieldCheck, ArrowRight, TrendingDown, Hash } from "lucide-react";
import { cn } from "@/lib/utils";
import { fetchAdminUsers, fetchAdminUserDetails, adjustUserCreditsApi, AdminUserItem, AdminUserDetailsResponse, AdminUserCourseDetail, } from "@/services/adminService";
export default function AdminUsersPage() {
    ...
    const handleAdjustSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSuccessToast(`Successfully ${adjustType === "ADD" ? "credited" : "deducted"} ${adjustAmount} credits for ${selectedUser.username}`);
    ...
    return (<div className="space-y-8 transition-colors duration-200">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-2">
          <button onClick={loadUsers} disabled={loading} className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 cursor-pointer">
            <RefreshCw className={cn("h-3.5 w-3.5", loading && "animate-spin")}/>
            Refresh
          </button>
          {successToast && (<div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/30 bg-emerald-500/10 px-3.5 py-2 text-xs font-semibold text-emerald-700 dark:text-emerald-400 animate-in fade-in">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} placeholder="Search by username or email..." className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-9 pr-4 text-xs text-slate-900 placeholder-slate-400 outline-none transition-all focus:border-lime-500 focus:ring-2 focus:ring-lime-500/20 dark:border-white/10 dark:bg-white/[0.03] dark:text-white dark:placeholder-white/40 dark:focus:border-lime-400"/>
        </div>

        <div className="flex items-center gap-2 max-md:flex-wrap">
          {["ALL", "Free", "Pro", "Team"].map((plan) => (<button key={plan} type="button" onClick={() => setPlanFilter(plan)} className={cn("rounded-xl px-3 py-1.5 text-xs font-semibold border transition-colors cursor-pointer", planFilter === plan
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-colors duration-200 dark:border-white/10 dark:bg-white/[0.03] dark:shadow-none">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            return (<tr key={user.id} onClick={() => handleOpenUserDetails(user)} className="group cursor-pointer hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-slate-900 group-hover:text-lime-600 dark:text-white dark:group-hover:text-lime-300 transition-colors flex items-center gap-1.5">
                              {user.username}
                              {user.isVerified && (<ShieldCheck className="h-3.5 w-3.5 text-lime-400" title="Verified User"/>)}
                            </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="space-y-1">
                          <span className={cn("inline-block rounded-md px-2 py-0.5 text-[10px] font-bold border", planName === "Team" && "bg-white/[0.08] text-white/80 border-white/15", planName === "Pro" && "bg-lime-400/10 text-lime-300 border-lime-400/25", planName === "Free" && "bg-slate-100 text-slate-600 border-slate-200 dark:bg-white/10 dark:text-white/60 dark:border-white/10")}>
                            {planName} Plan
                          </span>

                          <div className="flex flex-wrap items-center gap-1">
                            {(user.rechargeCount ?? 0) > 0 && (<span className="inline-flex items-center gap-0.5 rounded bg-white/[0.06] px-1.5 py-0.5 text-[9px] font-semibold text-white/65 border border-white/10">
                                <Zap className="h-2.5 w-2.5"/>
                                {user.rechargeCount} Top-up{(user.rechargeCount ?? 0) > 1 ? "s" : ""} (₹{user.totalRechargedINR})
                              </span>)}
                            {(user.planSubscriptionCount ?? 0) > 0 && (<span className="inline-flex items-center gap-0.5 rounded bg-white/[0.06] px-1.5 py-0.5 text-[9px] font-semibold text-white/65 border border-white/10">
                      ...
                      <td className="px-6 py-4">
                        ...
                        {reserved > 0 ? (<span className="text-[10px] font-semibold text-white/60">
                            ({reserved} cr hold)
                          </span>) : (<span className="text-[10px] text-slate-400 dark:text-white/40">Ready</span>)}
                      </td>
                      ...
                      <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-2">
                          <button type="button" onClick={() => handleOpenUserDetails(user)} className="inline-flex items-center gap-1 rounded-lg border border-lime-400/25 bg-lime-400/10 px-2.5 py-1 text-xs font-semibold text-lime-300 hover:bg-lime-400/20 cursor-pointer" title="Open Full User Profile, Courses & Deductions">
                            <Eye className="h-3.5 w-3.5"/>
                            View Details
                          </button>
                          <button type="button" onClick={() => setSelectedUser(user)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 cursor-pointer dark:border-white/10 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10" title="Adjust Credits">
      ...
      {activeUserId && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-3 sm:p-6 animate-in fade-in">
          <div className="relative flex flex-col w-full max-w-4xl max-h-[92vh] rounded-3xl border border-slate-200 bg-white shadow-2xl overflow-hidden dark:border-white/10 dark:bg-[#0b1220] dark:text-white">
            ...
            <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02] max-md:flex-col max-md:items-start max-md:gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {userDetails?.user?.isVerified && (<span className="inline-flex items-center gap-1 rounded-md bg-emerald-500/10 px-2 py-0.5 text-[10px] font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-500/20">
              ...
              <div className="flex items-center gap-2">
                {userDetails && (<button type="button" onClick={() => setSelectedUser(userDetails.user)} className="inline-flex items-center gap-1 rounded-xl border border-lime-500/30 bg-lime-500/10 px-3 py-1.5 text-xs font-bold text-lime-700 hover:bg-lime-500/20 cursor-pointer dark:border-lime-400/30 dark:bg-lime-400/10 dark:text-lime-400">
                    <Plus className="h-3.5 w-3.5"/> Adjust Credits
                  </button>)}
                <button type="button" onClick={handleCloseUserDetails} className="rounded-xl p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white cursor-pointer">
                  <X className="h-5 w-5"/>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-6 border-b border-slate-200 dark:border-white/10 bg-slate-50/25 dark:bg-white/[0.01]">
              <div className="rounded-2xl border border-slate-200 bg-white p-3.5 dark:border-white/10 dark:bg-white/[0.03]">
                ...
                <div className="text-[10px] text-slate-400 mt-0.5">
                  {(userDetails?.summary?.reservedCredits ?? 0) > 0 ? (<span className="text-amber-500 font-semibold">{userDetails?.summary?.reservedCredits} cr on hold</span>) : ("No pending hold")}
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 border-b border-slate-200 px-6 pt-3 dark:border-white/10 overflow-x-auto max-md:px-3">
              <button type="button" onClick={() => setActiveTab("courses")} className={cn("flex items-center gap-2 pb-3 px-1 text-xs font-bold border-b-2 transition-colors cursor-pointer", activeTab === "courses"
              ...
              <button type="button" onClick={() => setActiveTab("raw")} className={cn("flex items-center gap-2 pb-3 px-1 text-xs font-bold border-b-2 transition-colors cursor-pointer", activeTab === "raw"
                ? "border-lime-500 text-lime-700 dark:border-lime-400 dark:text-lime-400"
                : "border-transparent text-slate-500 hover:text-slate-800 dark:text-white/60 dark:hover:text-white")}>
              ...
              <button type="button" onClick={() => setActiveTab("ledger")} className={cn("flex items-center gap-2 pb-3 px-1 text-xs font-bold border-b-2 transition-colors cursor-pointer", activeTab === "ledger"
                ? "border-lime-500 text-lime-700 dark:border-lime-400 dark:text-lime-400"
                : "border-transparent text-slate-500 hover:text-slate-800 dark:text-white/60 dark:hover:text-white")}>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {detailsLoading ? (<div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-3">
                ...
                </div>) : detailsError ? (<div className="rounded-2xl border border-red-500/30 bg-red-500/10 p-5 text-xs font-semibold text-red-600 dark:text-red-400">
                  {detailsError}
                </div>) : (<>
                  ...
                  {activeTab === "courses" && (<div className="space-y-4">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div className="relative flex-1 max-w-sm">
                          ...
                          <input type="text" value={courseSearch} onChange={(e) => setCourseSearch(e.target.value)} placeholder="Filter user courses..." className="h-9 w-full rounded-xl border border-slate-200 bg-slate-50 pl-8 pr-3 text-xs text-slate-900 placeholder-slate-400 outline-none focus:border-lime-500 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-white"/>
                        </div>
                      ...
                      {displayedCourses.length > 0 ? (<div className="space-y-4">
                          {displayedCourses.map((course) => {
                            const isExpanded = expandedCourseId === course.id;
                            const hasGammaSlides = course.modules?.some((m) => m.gammaUrl);
                            return (<div key={course.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-xs transition-all dark:border-white/10 dark:bg-white/[0.02]">
                                ...
                                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                                  <div className="flex-1 space-y-1.5">
                                    <div className="flex flex-wrap items-center gap-2">
                                      ...
                                      {course.type && (<span className="rounded-md bg-purple-500/10 px-2 py-0.5 text-[10px] font-bold text-purple-600 dark:text-purple-400 border border-purple-500/20">
                                          {course.type}
                                        </span>)}
                                    </div>
                                ...
                                <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-3 dark:border-white/5">
                                  ...
                                  {course.breakdown.audioCredits > 0 && (<span className="inline-flex items-center gap-1.5 rounded-xl border border-amber-500/20 bg-amber-500/5 px-2.5 py-1 text-xs font-semibold text-amber-700 dark:text-amber-300">
                                  ...
                                  <div className="ml-auto flex items-center gap-2">
                                    {course.audioUrl && (<a href={course.audioUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-amber-500/10 px-2 py-1 text-[11px] font-bold text-amber-600 hover:bg-amber-500/20 dark:text-amber-400">
                                        <Headphones className="h-3 w-3"/> Audio ↗
                                      </a>)}
                                    {course.ebookUrl && (<a href={course.ebookUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg bg-blue-500/10 px-2 py-1 text-[11px] font-bold text-blue-600 hover:bg-blue-500/20 dark:text-blue-400">
                                        <FileText className="h-3 w-3"/> Ebook PDF ↗
                                      </a>)}

                                    <button type="button" onClick={() => setExpandedCourseId(isExpanded ? null : course.id)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 cursor-pointer dark:border-white/10 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10">
                                      {isExpanded ? (<>Hide Modules <ChevronUp className="h-3.5 w-3.5"/></>) : (<>Inspect Modules ({course.modules?.length || 0}) <ChevronDown className="h-3.5 w-3.5"/></>)}
                                    </button>
                                  </div>
                                </div>

                                {isExpanded && (<div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/75 p-4 dark:border-white/10 dark:bg-white/[0.02] space-y-3 animate-in fade-in">
                                    ...
                                    <div className="grid gap-2.5 sm:grid-cols-2">
                                      {course.modules && course.modules.length > 0 ? (course.modules.map((mod) => (<div key={mod.moduleId || mod.moduleNumber} className="flex flex-col justify-between gap-2 rounded-xl border border-slate-200 bg-white p-3 shadow-xs dark:border-white/10 dark:bg-white/[0.03]">
                                            <div className="flex items-start justify-between gap-2">
                                              ...
                                              <span className={cn("rounded-md px-1.5 py-0.5 text-[9px] font-bold uppercase", mod.status === "completed" || mod.gammaUrl
                                            ...
                                            <div className="flex items-center justify-between border-t border-slate-100 pt-2 text-[11px] text-slate-500 dark:border-white/5 dark:text-white/50">
                                              ...
                                              {mod.gammaUrl ? (<a href={mod.gammaUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 font-bold text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300">
                                                  Open Slides <ExternalLink className="h-3 w-3"/>
                                                </a>) : (<span className="text-slate-400 text-[10px]">No slides generated</span>)}
                                            </div>
                                          </div>))) : (<div className="col-span-2 text-center py-3 text-xs text-slate-400">
                                          No module objects recorded for this course.
                                        </div>)}
                                    </div>

                                    {course.transactions && course.transactions.length > 0 && (<div className="mt-3 border-t border-slate-200 pt-3 dark:border-white/10">
                                        <div className="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">
                                          Audited Ledger Entries for this Course ({course.transactions.length})
                                        </div>
                                        <div className="space-y-1.5 max-h-32 overflow-y-auto">
                                          {course.transactions.map((tx) => (<div key={tx.id} className="flex items-center justify-between text-[11px] rounded-lg bg-white p-2 border border-slate-200 dark:bg-white/[0.02] dark:border-white/5">
                        ...
                        </div>) : (<div className="flex flex-col items-center justify-center py-16 text-center rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
                  ...
                  {activeTab === "raw" && (<div className="space-y-4">
                      ...
                      <div className="overflow-hidden rounded-xl border border-slate-200 bg-white dark:border-white/10 dark:bg-white/[0.02]">
                        <div className="overflow-x-auto">
                          <table className="w-full text-left text-xs">
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                              {userDetails?.recentTransactions && userDetails.recentTransactions.length > 0 ? (userDetails.recentTransactions.map((tx) => {
                        const isPositive = tx.amount > 0 && (tx.type === "RECHARGE" || tx.type === "PLAN_RESET" || (tx.type === "ADJUSTMENT" && tx.amount > 0));
                        return (<tr key={tx.id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                                      <td className="px-4 py-3 font-bold">
                                        <span className={cn("rounded-md px-2 py-0.5 text-[10px] font-bold", tx.type === "RECHARGE" && "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400", tx.type === "PLAN_RESET" && "bg-purple-500/10 text-purple-600 dark:text-purple-400", tx.type === "RECONCILE" && "bg-red-500/10 text-red-600 dark:text-red-400", tx.type === "ADJUSTMENT" && "bg-amber-500/10 text-amber-600 dark:text-amber-400", tx.type === "RESERVE" && "bg-blue-500/10 text-blue-600 dark:text-blue-400")}>
                                          {tx.type}
                                        </span>
                                      </td>
            ...
            <div className="flex items-center justify-between border-t border-slate-200 px-6 py-4 dark:border-white/10 bg-slate-50/50 dark:bg-white/[0.02] max-md:flex-col max-md:items-stretch max-md:gap-3">
              ...
              <button type="button" onClick={handleCloseUserDetails} className="rounded-xl bg-slate-900 px-5 py-2 text-xs font-bold text-white hover:bg-slate-800 cursor-pointer dark:bg-white dark:text-black dark:hover:bg-slate-200">
                Close View
              </button>
            </div>
          </div>
      ...
      {selectedUser && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-[#0b1220] dark:text-white">
            <div className="flex items-center justify-between border-b border-slate-200 pb-4 dark:border-white/10">
              ...
              <button type="button" onClick={() => setSelectedUser(null)} className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10">
                <X className="h-4 w-4"/>
              </button>
            </div>
            ...
            <form onSubmit={handleAdjustSubmit} className="mt-4 space-y-4">
              <div>
                ...
                <div className="mt-1.5 grid grid-cols-2 gap-2">
                  <button type="button" onClick={() => setAdjustType("ADD")} className={cn("flex items-center justify-center gap-1.5 rounded-xl border py-2 text-xs font-semibold transition-colors cursor-pointer", adjustType === "ADD"
                ...
                : "border-slate-200 text-slate-600 dark:border-white/10 dark:text-white/60")}>
                  ...
                  <button type="button" onClick={() => setAdjustType("DEDUCT")} className={cn("flex items-center justify-center gap-1.5 rounded-xl border py-2 text-xs font-semibold transition-colors cursor-pointer", adjustType === "DEDUCT"
              ...
              <div>
                ...
                <div className="mt-1.5 flex items-center gap-2">
                  <input type="number" min="1" value={adjustAmount} onChange={(e) => setAdjustAmount(Number(e.target.value))} required className="h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 font-mono text-sm font-bold text-slate-900 outline-none focus:border-lime-500 focus:bg-white dark:border-white/10 dark:bg-white/5 dark:text-white dark:focus:border-lime-400"/>
              ...
              <div>
                ...
                <select value={adjustReason} onChange={(e) => setAdjustReason(e.target.value)} className="mt-1.5 h-10 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-900 outline-none dark:border-white/10 dark:bg-[#0b1220] dark:text-white">
              ...
              <div>
                ...
                <input type="text" value={customNote} onChange={(e) => setCustomNote(e.target.value)} placeholder="e.g. Ticket #4892 Gamma slide timeout" className="mt-1.5 h-9 w-full rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs text-slate-900 placeholder-slate-400 outline-none dark:border-white/10 dark:bg-white/5 dark:text-white dark:placeholder-white/40"/>
              </div>

              <div className="mt-6 flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => setSelectedUser(null)} className="rounded-xl border border-slate-200 px-4 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-100 cursor-pointer dark:border-white/10 dark:text-white/80 dark:hover:bg-white/10">
                  Cancel
                </button>
                <button type="submit" disabled={isSubmitting} className="rounded-xl bg-lime-500 px-4 py-2 text-xs font-bold text-black hover:bg-lime-400 cursor-pointer shadow-md disabled:opacity-50">
