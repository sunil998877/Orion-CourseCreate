...
export default function AdminPricingPage() {
  ...
  const saveEdit = async (id: string) => {
    try {
      ...
      setSkus((prev) =>
        prev.map((sku) => (sku._id === id ? { ...sku, creditCost: editCost } : sku))
      );
      setEditingId(null);
  ...
  const toggleStatus = async (sku: AdminPricingRuleItem) => {
    try {
      ...
      setSkus((prev) =>
        prev.map((s) => (s._id === sku._id ? { ...s, isActive: newStatus } : s))
      );
  ...
  return (
    <div className="space-y-8 transition-colors duration-200">
      ...
      {error && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs text-amber-700 dark:text-amber-400 font-semibold">
          Notice: {error}
        </div>
      )}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-white/5">
              {skus.length > 0 ? (
                skus.map((sku) => {
                  ...
                  return (
                    <tr key={sku._id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={cn(
                            "inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[10px] font-bold uppercase border",
                            sku.provider === "gamma" && "bg-purple-500/10 text-purple-600 border-purple-500/20 dark:text-purple-400",
                            sku.provider === "openai" && "bg-emerald-500/10 text-emerald-600 border-emerald-500/20 dark:text-emerald-400",
                            sku.provider === "elevenlabs" && "bg-amber-500/10 text-amber-600 border-amber-500/20 dark:text-amber-400"
                          )}
                        >
                        ...
                        {isEditing ? (
                          <div className="flex items-center gap-2">
                            <input
                              ...
                              className="w-20 rounded-lg border border-lime-500 bg-white px-2 py-1 font-mono text-xs font-bold text-slate-900 outline-none dark:border-lime-400 dark:bg-slate-900 dark:text-white"
                            />
                            <span className="text-[11px] text-slate-400">cr</span>
                          </div>
                        ) : (
                          <div className="font-mono text-sm font-bold text-lime-600 dark:text-lime-400">
                      ...
                      <td className="px-6 py-4">
                        <button
                          ...
                          className={cn(
                            "inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-[10px] font-bold border transition-colors cursor-pointer",
                            sku.isActive
                      ...
                      <td className="px-6 py-4 text-right">
                        {isEditing ? (
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              type="button"
                              onClick={() => saveEdit(sku._id)}
                              className="flex h-7 w-7 items-center justify-center rounded-lg bg-lime-500/10 text-lime-600 border border-lime-500/30 hover:bg-lime-500/20 cursor-pointer dark:text-lime-400"
                              title="Save"
                            >
                              <Check className="h-4 w-4" />
                            </button>
                            <button
                              ...
                              className="flex h-7 w-7 items-center justify-center rounded-lg bg-slate-100 text-slate-600 border border-slate-200 hover:bg-slate-200 cursor-pointer dark:bg-white/5 dark:text-white/60 dark:border-white/10"
                              title="Cancel"
                            >
                              <X className="h-4 w-4" />
                            </button>
                          ...
                          <button
                            ...
                            className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-200 cursor-pointer dark:border-white/10 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10"
                          >
