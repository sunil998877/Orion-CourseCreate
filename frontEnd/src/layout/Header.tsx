...
export default function Header({ onOpenMobileMenu, userInfo, avatarUrl, notifications, onOpenAvatarModal, onOpenChangePasswordModal, onLogout, onMarkAllRead, onRemoveAllNotifications, onFetchNotifications, }: HeaderProps) {
    ...
    return (<header className="fixed top-0 left-0 right-0 z-50 bg-black/30 backdrop-blur-lg border-b border-white/10 h-16">
      <div className="h-16 px-3 md:px-6 flex items-center justify-between gap-2 md:gap-6">
        ...
        <div className="flex-1 max-w-2xl">
          <div className="relative">
            ...
            <input id="search" type="text" placeholder="Search courses" className="w-full h-9 md:h-10 rounded-full bg-white/5 border border-white/10 pl-9 md:pl-10 pr-3 md:pr-4 text-xs md:text-sm text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-lime-500 transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={(e) => {
            if (e.key === 'Enter')
                triggerSearch();
        }}/>
          </div>
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          ...
          <div className="relative" ref={notifDropdownRef}>
            <button className={`p-2 rounded-full border transition-all relative ${notifOpen ? 'bg-white/15 border-white/20' : 'bg-white/5 border-white/10 hover:bg-white/10 '}`} onClick={() => {
            setNotifOpen((v) => !v);
            if (!notifOpen)
                onFetchNotifications();
        }} aria-label="Notifications">
              <Bell className={`w-5 h-5 transition-all ${notifOpen ? 'text-white' : 'text-white/70'}`}/>
              {notifications.some((n) => !n.isRead) && (<span className="absolute top-1 right-2 w-2 h-2 bg-lime-500 rounded-full animate-pulse"/>)}
            </button>

            {notifOpen && (<div className="absolute top-full right-0 mt-2 z-50 w-80 max-md:fixed max-md:left-3 max-md:right-3 max-md:w-auto max-md:top-16 bg-[#0A0F1A]/95 border border-white/20 backdrop-blur-md rounded-xl shadow-xl overflow-hidden">
                <div className="px-4 py-3 border-b border-white/10 flex items-center justify-between max-md:flex-col max-md:items-start max-md:gap-2">
                  ...
                  <div className="flex gap-2">
                    <button className="text-xs px-2 py-1 rounded bg-lime-500/30 text-lime-200 hover:bg-lime-500/40 transition" onClick={(e) => {
                e.stopPropagation();
                onMarkAllRead();
            }}>
                      Mark all read
                    </button>
                    <button className="text-xs px-2 py-1 rounded bg-red-500/30 text-red-200 hover:bg-red-500/40 transition" onClick={(e) => {
                e.stopPropagation();
          ...
          <div className="relative" ref={avatarDropdownRef}>
            <button className="rounded-full focus:outline-none transition-transform active:scale-95" onClick={() => setAvatarDropdownOpen(!avatarDropdownOpen)} aria-label="User menu">
              {avatarUrl ? (<img src={avatarUrl} alt="avatar" className="w-8 h-8 md:w-10 md:h-10 rounded-full object-cover border border-white/10"/>) : (<div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/10 flex items-center justify-center border border-white/10">
                  <User className="w-5 h-5 md:w-6 md:h-6 text-white/70"/>
                </div>)}
            </button>

            {avatarDropdownOpen && (<div className="absolute top-full right-0 mt-2 z-50 w-64 max-md:w-[min(16rem,calc(100vw-1.5rem))] bg-[#1a1f2e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-fadeInUp">
