...
const CreditsTracker: React.FC<CreditsTrackerProps> = ({ collapsed = false }) => {
    ...
    return (
      <div ref={containerRef} className="relative mt-auto border-t border-white/10 p-2">
        ...
        <button
          ...
          title="Credits"
          className="mx-auto flex h-10 w-10 items-center justify-center rounded-lg border border-lime-500/20 bg-lime-500/10 text-lime-400 transition hover:bg-lime-500/20"
        >
          <Zap size={16} />
        </button>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-black/20 backdrop-blur-sm transition hover:border-lime-500/20">
        ...
        >
          ...
          <div className="mb-2 flex items-end justify-between">
            <div>
              <span className="text-lg font-bold text-white">{credits.remaining.toLocaleString()}</span>
              <span className="ml-1 text-xs text-white/40">/ {credits.total.toLocaleString()}</span>
            </div>
