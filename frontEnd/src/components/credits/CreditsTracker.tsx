...
const CreditsTracker: React.FC = () => {
    ...
    return (<div ref={containerRef} className="relative mt-auto border-t border-white/10 p-3 max-md:p-2">
      {popoverOpen && (<CreditsPopover credits={credits} usagePercentage={usagePercentage} usageHistory={transactions} onClose={() => setPopoverOpen(false)} onAddCredits={goToAddCredits}/>)}

      <div className="rounded-xl border border-white/10 bg-black/20 backdrop-blur-sm transition hover:border-lime-500/20">
        <button type="button" onClick={() => setPopoverOpen((prev) => !prev)} className="w-full rounded-t-xl p-3 text-left transition hover:bg-white/5">
        ...
        <div className="border-t border-white/5 p-2">
          <button type="button" onClick={goToAddCredits} className="flex w-full items-center justify-center gap-1.5 rounded-lg border border-lime-500/20 bg-lime-500/10 px-3 py-2 text-xs font-bold text-lime-400 transition hover:bg-lime-500/20 hover:text-lime-300">
            <Plus size={14}/>
            Add Credits
          </button>
        </div>
