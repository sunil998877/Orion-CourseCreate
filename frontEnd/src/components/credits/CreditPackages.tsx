...
const CreditPackages: React.FC<CreditPackagesProps> = ({ onSelectPackage, selectedPackageId, isProcessing = false, }) => {
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {CREDIT_PACKAGES.map((pkg) => (<CreditPackageCard key={pkg.id} pkg={pkg} onSelect={onSelectPackage} isLoading={isProcessing && selectedPackageId === pkg.id} isDisabled={isProcessing}/>))}
      </div>
