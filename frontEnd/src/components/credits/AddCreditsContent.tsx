...
const AddCreditsContent: React.FC<Props> = ({ onPurchase }) => {
    ...
    return (<div className="space-y-12">
      ...
      <section id="recharge" className="rounded-[2rem] border border-white/10 bg-white/[0.02] p-6 md:p-8 max-md:rounded-2xl max-md:p-4">
        <CreditPackages onSelectPackage={handleSelectPackage} selectedPackageId={selectedPackage?.id}/>
      </section>

      {selectedPackage && (<CreditsPurchase pkg={selectedPackage} onClose={handleClosePackagePurchase} onSuccess={handlePackagePurchaseSuccess}/>)}

      {selectedPlan && (<PlanPurchaseModal plan={selectedPlan} onClose={handleClosePlanPurchase}/>)}
    </div>);
};
