...
const getActionName = (tx: WalletTransaction): string => {
    if (tx.action?.displayName)
        return tx.action.displayName;
    if (tx.action_name)
        return tx.action_name;
    if (tx.type === 'RECHARGE')
        return 'Wallet Top-Up';
    if (tx.type === 'REFUND')
        return 'Credit Refund';
    if (tx.type === 'PLAN_RESET')
        return 'Monthly Plan Reset';
...
const UsageHistory: React.FC<Props> = ({ items, limit, compact = false, maxHeight }) => {
    ...
    return (<div className={compact ? 'space-y-2' : 'space-y-3'}>
      <div className="flex items-center justify-between">
        ...
        {items.length > 0 && (<span className="text-[10px] font-medium text-white/40">
            {items.length} {items.length === 1 ? 'entry' : 'entries'}
          </span>)}
      </div>

      {items.length === 0 ? (<p className="text-xs text-white/40 py-2">No transactions yet.</p>) : (<div className={`space-y-1.5 overflow-y-auto pr-1 scroll-smooth ${containerMaxHeight} [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/10 hover:[&::-webkit-scrollbar-thumb]:bg-lime-400/40`}>
          {displayedItems.map((tx, idx) => {
                ...
                return (<div key={key} className="flex items-center justify-between rounded-lg bg-white/[0.03] border border-white/5 px-3 py-2 transition-colors hover:bg-white/[0.06]">
                <div className="min-w-0 flex items-center gap-2">

                  <span className={`shrink-0 rounded-full border px-2 py-0.5 text-[9px] font-bold tracking-wide uppercase ${badgeStyle}`}>
                    {badgeLabel}
                  </span>
