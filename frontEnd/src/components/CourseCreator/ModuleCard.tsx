...
const ModuleCard: React.FC<{
    ...
    const { moduleRefs, highlightedModuleId, moduleCredits, courseData, themeByModule, setSelectedModuleForTheme, setIsThemeModalOpen, openContentPreview, openSlidesPreview, isPreviewLoading, orionUrlByModule, handleGenerateSlidesOrion, generatingSlidesModuleId, slideGenerationProgress } = useCourseCreator();
    return (<div key={mod.id} ref={(el) => (moduleRefs.current[mod.id] = el)} className={`bg-gray-900/40 backdrop-blur-md p-10 md:p-12 rounded-[2.5rem] border flex flex-col hover:border-lime-500/30 transition-all duration-300 group/card shadow-2xl hover:shadow-lime-500/10 max-md:rounded-2xl max-md:p-4 ${highlightedModuleId === mod.id ? 'animate-blink-module' : 'border-gray-700/30'}`}>
                                ...
                                <div className="space-y-2 mb-8 flex-1">
                                  {mod.lessons.map((lesson: any, idx: any) => (<p key={idx} className="text-base text-gray-400 flex items-center gap-4 group/lesson transition-colors hover:text-gray-200 py-1">
            ...
            return (<div className="flex-1 flex flex-col">
                ...
                }} className="px-3 py-1.5 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs font-bold text-white transition-colors" type="button">
                                            Change
                                          </button>
                                        </div>

                                        <div className="flex gap-3">
                                          <button onClick={() => openContentPreview(mod.id)} disabled={isPreviewLoading} className="flex-1 flex items-center justify-center gap-2 py-3 text-[10px] font-black uppercase tracking-wider bg-gray-800/40 hover:bg-gray-800 border border-gray-700/30 hover:border-gray-600 rounded-2xl text-gray-400 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed group/btn" type="button">
                                            <BookOpen size={14} className="text-lime-500 group-hover/btn:scale-110 transition-transform"/> View
                                          </button>
                                          <button onClick={() => openSlidesPreview(mod.id, false)} disabled={isPreviewLoading} className="flex-1 flex items-center justify-center gap-2 py-3 text-[10px] font-black uppercase tracking-wider bg-gray-800/40 hover:bg-gray-800 border border-gray-700/30 hover:border-gray-600 rounded-2xl text-gray-400 hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed group/btn" type="button">
                                            <Eye size={14} className="text-lime-500 group-hover/btn:scale-110 transition-transform"/> Slides
                                          </button>
                                        </div>
                                        <button onClick={() => orionUrlByModule[mod.id] ? openSlidesPreview(mod.id, true) : handleGenerateSlidesOrion(mod.id)} disabled={isPreviewLoading || generatingSlidesModuleId !== null} className={`w-full relative overflow-hidden flex items-center justify-center gap-2 py-4 text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl border transition-all disabled:opacity-50 disabled:cursor-not-allowed ring-offset-2 ring-offset-black group shadow-xl ${orionUrlByModule[mod.id]
                    ? 'bg-lime-500/10 border-lime-500/30 text-lime-400 hover:bg-lime-500/20 hover:border-lime-500/50'
                    : 'bg-white text-black border-white hover:bg-lime-400 hover:border-lime-400'}`} type="button">
                                          {generatingSlidesModuleId === mod.id && (<div className="absolute inset-0 bg-lime-500/20 transition-all duration-300 ease-out z-0" style={{ width: `${slideGenerationProgress}%` }}/>)}
