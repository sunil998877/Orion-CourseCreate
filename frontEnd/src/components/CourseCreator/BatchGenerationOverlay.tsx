...
const BatchGenerationOverlay: React.FC = () => {
    const { isBatchGenerating, batchSlidesProgress, batchSlidesDisplayProgress, batchGeneratingModuleId, batchSelectedModuleIdForPreview, previewModules, orionUrlByModule, setBatchSelectedModuleIdForPreview } = useCourseCreator();
    return (<AnimatePresence>
            {isBatchGenerating && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-8 bg-black/85 backdrop-blur-xl">
                    <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="bg-[#0B0C10] border border-white/[0.08] w-full max-w-7xl h-[88vh] rounded-[2.5rem] p-6 sm:p-8 shadow-[0_24px_60px_-15px_rgba(0,0,0,0.8)] relative overflow-hidden flex flex-col md:flex-row gap-8 backdrop-blur-3xl">
                        ...
                        <div className="w-full md:w-[38%] flex flex-col h-full min-h-0 border-r border-white/5 pr-0 md:pr-8">
                            <div className="relative shrink-0 flex items-center gap-5 mb-6">
                                ...
                                <div className="relative w-24 h-24 shrink-0">
                                    ...
                                    <div className="absolute inset-0 rounded-full transition-all duration-500 ease-out shadow-[0_0_15px_rgba(132,204,22,0.15)]" style={{
                ...
                mask: 'radial-gradient(transparent 64%, black 65%)'
                            ...
                            <div className="shrink-0 mb-6 bg-gray-900/40 border border-white/5 p-4 rounded-2xl">
                                ...
                                <div className="w-full bg-gray-800/40 rounded-full h-2 overflow-hidden border border-gray-800 p-0.5">
                                    <div className="bg-gradient-to-r from-lime-500 via-emerald-500 to-teal-500 h-full rounded-full transition-all duration-700 ease-out shadow-[0_0_12px_rgba(132,204,22,0.4)]" style={{ width: `${batchSlidesDisplayProgress}%` }}/>
                                </div>
                            </div>
                ...
                return (<button key={mod.id} onClick={() => isGenerated && setBatchSelectedModuleIdForPreview(mod.id)} className={`w-full text-left p-4 rounded-2xl border transition-all duration-300 flex items-center justify-between gap-4 group/item ${isActive
                        ...
                        : isGenerated
                            ...
                            : 'border-gray-800/40 bg-gray-900/10 cursor-not-allowed opacity-60'}`} disabled={!isGenerated}>
                            ...
                            : 'bg-gray-800 text-gray-500'}`}>
                                            ...
                                            <div className="shrink-0 flex items-center gap-2">
                                                {isGenerated ? (<span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-[9px] font-black uppercase text-emerald-400">
                                                    ...
                                                    </span>) : isGenerating ? (<span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-lime-500/10 border border-lime-500/20 text-[9px] font-black uppercase text-lime-400 animate-pulse">
                                                        <Loader2 className="w-3 h-3 animate-spin"/> Baking
                                                    </span>) : (<span className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-gray-800 border border-gray-700 text-[9px] font-black uppercase text-gray-500">
                ...
                if (activeUrl) {
                    return (<div className="w-full h-full flex flex-col relative animate-in fade-in duration-500">
                                            ...
                                            <iframe src={activeUrl.replace('/docs/', '/embed/').replace('/view/', '/embed/')} className="w-full h-full border-0 bg-transparent relative z-0" allowFullScreen title="Generated Slide deck preview"/>
                                        </div>);
                }
                else {
                    return (<div className="flex-1 flex flex-col items-center justify-center p-8 text-center relative overflow-hidden h-full">
