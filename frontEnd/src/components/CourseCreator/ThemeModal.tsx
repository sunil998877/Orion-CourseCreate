...
const ThemeModal: React.FC = () => {
    const { isThemeModalOpen, selectedModuleForTheme, setIsThemeModalOpen, setSelectedModuleForTheme, themeFilter, setThemeFilter, themeByModule, setThemeByModule, courseData, updateCourseData } = useCourseCreator();
    if (!isThemeModalOpen)
        return null;
    return (<>
            ...
            setSelectedModuleForTheme(null);
        }} className="p-2 text-gray-500 hover:text-white bg-gray-900 hover:bg-gray-800 rounded-xl transition-all" type="button">
                            <X size={20}/>
                        </button>
                    </div>
                    ...
                    <div className="p-6 overflow-y-auto custom-scrollbar">
                        ...
                        <div className="flex flex-wrap gap-2 mb-8">
                            {THEME_CATEGORIES.map(cat => (<button key={cat} onClick={() => setThemeFilter(cat)} className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all ${themeFilter === cat ? 'bg-transparent text-white border border-gray-600' : 'bg-transparent text-gray-500 border border-gray-800 hover:border-gray-600 hover:text-gray-300'}`} type="button">
                                    {cat}
                                </button>))}
                        </div>
                ...
                : 'border-gray-800/80 hover:border-gray-600 bg-[#171a21]'}`} type="button">
                                    <div className={`w-full h-20 rounded-xl mb-4 ${theme.gradient} shadow-inner flex items-center justify-center`}>
                                        <div className="flex gap-1.5">
                                            {theme.colors.slice(0, 3).map((c, i) => (<div key={i} className="w-4 h-4 rounded-full border border-white/20 shadow-sm" style={{ backgroundColor: c }}/>))}
                                        </div>
                                    </div>
                                    <div className="flex flex-col w-full">
                                        ...
                                        <div className="flex flex-wrap gap-1">
                                            {theme.style.split(', ').map(tag => (<span key={tag} className="px-1.5 py-0.5 rounded bg-black/40 text-[8px] font-bold text-gray-400 uppercase tracking-widest">
                                                    {tag}
                                                </span>))}
                                        </div>
                                    </div>
