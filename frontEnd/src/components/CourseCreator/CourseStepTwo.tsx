...
const CourseStepTwo: React.FC = () => {
    const { step, courseData, updateCourseData, showValidation, isDescriptionEditable, setIsDescriptionEditable, setIsDescriptionModalOpen, isRefiningDescription, refinePromptOpen, setRefinePromptOpen, refinePromptText, setRefinePromptText, goToNextStep, goToPrevStep, handleRefineDescription, containerVariants, itemVariants, stepVariants } = useCourseCreator();
    return (<motion.div key="step2" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="pt-6 flex flex-col xl:flex-row gap-8 xl:gap-12 min-h-[600px]">
                  <div className="flex-1 xl:w-[55%] bg-[#0A0A0B]/60 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group space-y-8 max-md:rounded-2xl max-md:p-4">
                    ...
                    <div>
                      <div className="mb-2 flex justify-between items-center">
                        ...
                        {showValidation && (courseData.description?.trim().split(/\s+/).filter(Boolean).length || 0) >= 5000 && (<span className="text-xs font-bold text-amber-500 animate-pulse uppercase tracking-widest flex items-center gap-1">
                            <Zap size={12} fill="currentColor"/> Maximum limit reached
                          </span>)}
                      </div>
                      <div className="relative group">
                        <button type="button" onClick={() => setIsDescriptionModalOpen(true)} className="absolute left-3 top-4 z-10 p-1.5 rounded-lg transition-all bg-gray-800/80 text-gray-500 hover:text-white hover:bg-gray-700" title="Edit description in full view">
                          <Pencil className="w-4 h-4"/>
                        </button>
                        <textarea readOnly={!isDescriptionEditable || isRefiningDescription} className={`w-full bg-gray-800/50 border rounded-2xl p-4 pl-12 min-h-[160px] ${isDescriptionEditable ? 'pb-16' : ''} focus:ring-2 focus:ring-lime-500 outline-none transition-all resize-none ${!isDescriptionEditable || isRefiningDescription ? 'cursor-default text-gray-400' : 'cursor-text text-white'} ${showValidation && (courseData.description?.trim().split(/\s+/).filter(Boolean).length || 0) >= 5000
            ...
            : showValidation && (!courseData.description?.trim() || courseData.description.trim().split(/\s+/).filter(Boolean).length < 50)
                ? 'border-amber-500/50 ring-1 ring-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]'
                : 'border-gray-700'}`} placeholder="Describe the primary learning outcomes... (Minimum 50 words required)" value={courseData.description} onChange={(e) => {
            ...
            if (words.length <= 5000) {
                ...
                if (val.length < (courseData.description?.length || 0)) {
                    updateCourseData({ description: capitalized });
                        ...
                        {isRefiningDescription && (<div className="absolute inset-0 bg-gray-900/60 backdrop-blur-sm rounded-2xl flex items-center justify-center border border-lime-500/30 z-10">
                        ...
                        {showValidation && ((!courseData.description?.trim() || courseData.description.trim().split(/\s+/).filter(Boolean).length < 50) ||
            (courseData.description?.trim().split(/\s+/).filter(Boolean).length || 0) >= 5000) && (<div className="absolute right-4 top-4 text-amber-500 animate-pulse pointer-events-none">
                              <Zap size={18} fill="currentColor"/>
                            </div>)}
                        {isDescriptionEditable && !isRefiningDescription && (<div className="absolute right-4 bottom-4 animate-in fade-in zoom-in-95 duration-200 z-10">
                            <button type="button" onClick={() => setIsDescriptionEditable(false)} className="flex items-center gap-2 bg-lime-500 hover:bg-lime-400 text-black px-4 py-2 rounded-xl text-xs font-black shadow-lg shadow-lime-500/30 transition-all uppercase tracking-wide">
                              <Check size={16} strokeWidth={3}/> SAVE
                            </button>
                          </div>)}
                      </div>

                      {refinePromptOpen && (<div className="mt-3 bg-gray-800/80 p-3 rounded-xl border border-lime-500/30 animate-in fade-in zoom-in-95 duration-200">
                          ...
                          <div className="flex gap-2 max-md:flex-col">
                            <input type="text" value={refinePromptText} onChange={(e) => setRefinePromptText(e.target.value)} placeholder="e.g., Make it shorter, focus more on beginners..." className="flex-1 bg-gray-900/50 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-lime-500 focus:ring-1 focus:ring-lime-500" onKeyDown={(e) => {
                if (e.key === 'Enter')
                    handleRefineDescription();
            }}/>
                            <button onClick={handleRefineDescription} disabled={isRefiningDescription || !refinePromptText.trim()} className="bg-lime-500 hover:bg-lime-400 text-black px-4 py-2 rounded-lg text-sm font-bold shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all">
                              {isRefiningDescription ? <Loader2 className="w-4 h-4 animate-spin"/> : <Sparkles className="w-4 h-4"/>}
                              Refine
                            </button>
                            <button onClick={() => { setRefinePromptOpen(false); setRefinePromptText(''); }} className="bg-gray-700 hover:bg-gray-600 text-white px-3 py-2 rounded-lg text-sm transition-all">
                              <X className="w-4 h-4"/>
                            </button>
                          </div>
                        </div>)}
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <button type="button" onClick={() => setRefinePromptOpen(!refinePromptOpen)} className="flex items-center gap-1.5 text-xs text-lime-400 hover:text-lime-300 font-semibold uppercase tracking-wider transition-colors">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                      <div className="space-y-4">
                        <div className="flex items-center justify-between gap-2 mb-2">
                          ...
                          {courseData.level && (<span className="text-[10px] font-black text-lime-400 bg-lime-400/10 px-2 py-0.5 rounded border border-lime-400/20 animate-in fade-in zoom-in duration-300 flex items-center gap-1">
                              <Sparkles size={10}/> RECOMMENDED BY ORION
                            </span>)}
                        </div>
                        <div className="relative">
                          <input type="number" min="0" className={`w-full bg-gray-800/50 border rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-lime-500 outline-none transition-all ${showValidation && (!(courseData.duration?.value) || (courseData.duration?.value ?? 0) <= 0) ? 'border-amber-500/50 ring-1 ring-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-gray-700'}`} placeholder="e.g. 10" value={courseData.duration?.value === 0 ? '' : (courseData.duration?.value ?? '')} onChange={(e) => {
            ...
            updateCourseData({
                ...
                module: recommendedModules
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="relative">
                          <input type="number" min="0" className={`w-full bg-gray-800/50 border rounded-xl py-3 px-4 text-white focus:ring-2 focus:ring-lime-500 outline-none transition-all ${showValidation && (courseData.module === undefined || courseData.module === null || courseData.module <= 0) ? 'border-amber-500/50 ring-1 ring-amber-500/20 shadow-[0_0_15px_rgba(245,158,11,0.1)]' : 'border-gray-700'}`} placeholder="e.g. 4" value={courseData.module === 0 ? '' : courseData.module} onChange={(e) => {
            ...
            updateCourseData({ module: val });
                    ...
                    {courseData.level && (<div className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-4 flex items-start gap-3 animate-in fade-in slide-in-from-top-2 duration-500">
                    ...
                    <div className="mt-12 flex justify-between items-center max-md:mt-8 max-md:gap-3">
                      <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={goToPrevStep} className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-gray-400 hover:text-white transition-all bg-white/5 hover:bg-white/10 max-md:flex-1 max-md:justify-center max-md:px-3" type="button">
                        <ChevronLeft size={20}/> Back
                      </motion.button>
                      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={goToNextStep} className="flex items-center gap-2 bg-gradient-to-r from-lime-500 to-emerald-500 text-black px-8 py-3 rounded-xl font-black shadow-lg shadow-lime-500/20 hover:shadow-lime-500/40 transform hover:-translate-y-0.5 max-md:flex-1 max-md:justify-center max-md:px-3" type="button">
                  ...
                  <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.2 }} className="flex-1 xl:w-[45%] bg-gradient-to-br from-[#0D0D15] via-[#0A0A0E] to-[#050505] rounded-[2.5rem] p-6 sm:p-9 border border-white/[0.08] shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group self-start sticky top-8 max-md:rounded-2xl max-md:p-4 max-md:static">
