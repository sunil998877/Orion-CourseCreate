            ...
            <div className="flex-1 xl:w-[55%] bg-[#0A0A0B]/60 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group flex flex-col h-full max-md:rounded-2xl max-md:p-4">
                <div className="grid lg:grid-cols-3 gap-8 mb-10 overflow-y-auto max-h-[500px] pr-2 custom-scrollbar flex-1">
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        {previewModules.map((mod: any) => (<div key={mod.id} className="group bg-gray-800/30 hover:bg-gray-800/60 p-6 rounded-3xl border border-gray-800 transition-all">
                                ...
                                {prefetchedSlidesMap[mod.id]?.Slides?.length > 0 && (<div className="pt-2 flex gap-2">
                                        {orionUrlByModule[mod.id] && (<button onClick={() => openSlidesPreview(mod.id, true)} disabled={isPreviewLoading} className="flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase rounded-xl border border-lime-500/20 text-lime-400 bg-lime-500/10 hover:bg-lime-500/20 transition-all disabled:opacity-50" type="button">
                                                <Book size={14}/>
                                                View Deck
                                            </button>)}
                                        <button onClick={() => downloadModulePPTX(mod.id)} disabled={downloadingModuleId === mod.id} className="flex-1 flex items-center justify-center gap-2 py-2 text-[10px] font-black uppercase bg-lime-500/10 hover:bg-lime-500/20 rounded-xl border border-lime-500/20 text-lime-400 transition-all disabled:opacity-50" type="button">
                                            {downloadingModuleId === mod.id ? (<Loader2 size={14} className="animate-spin"/>) : (<Download size={14}/>)}
