...
import { ChevronRight, ChevronLeft, Zap, Sparkles, AlertTriangle, Construction, Lightbulb, RefreshCw, Layers, Rocket } from 'lucide-react';
const CourseStepFour: React.FC = () => {
    const { courseData, isBlueprinting, hasBlueprint, previewModules, blueprintingProgress, setThemeByModule, showGenerateWarning, setShowGenerateWarning, goToNextStep, goToPrevStep, generateOrionPreview, stepVariants, containerVariants, itemVariants } = useCourseCreator();
    return (<motion.div key="step4" variants={stepVariants} initial="hidden" animate="visible" exit="exit" className="pt-6 flex flex-col xl:flex-row gap-8 xl:gap-12 min-h-[600px] h-full max-md:min-h-0">
            ...
            <div className="flex-1 xl:w-[66%] bg-[#0A0A0B]/60 backdrop-blur-xl border border-white/5 rounded-[2.5rem] p-8 md:p-12 shadow-2xl relative overflow-hidden group flex flex-col h-full max-md:rounded-2xl max-md:p-4">
                {!hasBlueprint && !isBlueprinting ? (<div className="flex-1 flex flex-col h-full">
                    ...
                    generateOrionPreview();
                        ...
                        <div className="relative w-44 h-44 mb-10">
                            ...
                            <div className="absolute inset-0 rounded-full transition-all duration-700 ease-out shadow-[0_0_20px_rgba(132,204,22,0.15)]" style={{
                ...
                mask: 'radial-gradient(transparent 64%, black 65%)'
                        ...
                        <div className="w-full max-w-md">
                            ...
                            <div className="w-full bg-gray-800/50 rounded-full h-3 overflow-hidden border border-gray-700 p-0.5 shadow-inner">
                                <div className="bg-gradient-to-r from-lime-500 via-emerald-500 to-teal-500 h-full rounded-full transition-all duration-700 ease-out shadow-[0_0_15px_rgba(132,204,22,0.4)]" style={{ width: `${blueprintingProgress}%` }}/>
                            </div>
                ...
                setThemeByModule(shuffled);
            }} className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-gray-400 hover:text-lime-400 rounded-xl border border-white/10 transition-all font-bold text-[10px] uppercase tracking-widest group" type="button">
                        ...
                        <div className="mt-auto pt-8 flex justify-end max-md:justify-stretch">
                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={goToNextStep} className="flex items-center gap-2 bg-lime-500 hover:bg-lime-400 text-black px-8 py-3 rounded-xl font-black shadow-lg shadow-lime-500/20 transition-all max-md:w-full max-md:justify-center" type="button">
