...
export const PodcastPlayer: React.FC<{
    podcastUrl?: string;
    player: any;
}> = ({ podcastUrl, player }) => <div className="animate-fadeInRight flex flex-col gap-4 p-5 bg-white/5 backdrop-blur-xl rounded-2xl border border-white/10 shadow-2xl w-80 lg:w-96 mt-3">
  <audio ref={player.podcastAudioRef} src={`${ORIGIN}${podcastUrl || ''}`} onTimeUpdate={player.handlePodcastTimeUpdate} onLoadedMetadata={player.handlePodcastLoadedMetadata} onEnded={() => player.setIsPodcastPlaying(false)}/>
    ...
    <div className="flex items-center justify-between pt-2 border-t border-white/5">
      <div className="flex items-center gap-2">
        <div className="relative">
          <button onClick={() => player.setShowPodcastSpeedMenu(!player.showPodcastSpeedMenu)} className="flex items-center gap-1.5 px-2 py-1 rounded-md bg-white/5 text-white/40 hover:text-lime-400">
        ...
        <div className="absolute bottom-full left-0 mb-2 p-1 bg-[#0b1220]/95 border border-white/10 rounded-lg z-50 flex flex-col">
            {[0.5, 1, 1.25, 1.5, 2].map((s: number) => <button key={s} onClick={() => player.selectPodcastSpeed(s)} className="px-4 py-1.5 text-[10px] text-white/60 hover:bg-white/10 text-left">{s === 1 ? 'Normal' : `${s}x`}
            </button>)}
          </div>}
        </div>
