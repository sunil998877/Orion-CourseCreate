...
export const CourseList: React.FC<{
  ...
  onCreateNew: () => void;
}> = ({ courses, loading = false, isSearching, onCourseClick, onDelete, onCreateNew }) => {
      ...
      >
        {loading && !courses.length
          ? Array.from({ length: 3 }).map((_, i) => (
              <div
                key={`skeleton-${i}`}
                className="min-h-[300px] animate-pulse rounded-3xl border border-white/10 bg-white/[0.04]"
              >
          ...
          : courses.map((course, idx) => (
              <CourseCard
                key={course._id || course.courseId || idx}
                course={course}
                index={idx}
