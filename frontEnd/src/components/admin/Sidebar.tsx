...
export default function Sidebar() {
    ...
    const logoMark = (<Link to="/admin" className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-white/15 dark:bg-[#111827]" aria-label="Orion AI">
    ...
    return (<TooltipProvider delay={100}>
      <SidebarHeader className="h-16 shrink-0 overflow-visible border-b border-[var(--admin-border)] p-0">
        <div className={cn("flex h-16 items-center gap-3 px-3", isCollapsed && "justify-center px-0")}>
          {logoMark}
          {!isCollapsed && (<div className="min-w-0 flex flex-col">
      ...
      <SidebarContent className={cn("bg-transparent p-3", isCollapsed && "px-2 py-4")}>
        <nav className="space-y-1.5">
          {menuItems.map((item) => {
            ...
            const linkContent = (<Link to={item.href} onClick={() => setOpenMobile(false)} className={cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200", isCollapsed && "mx-auto h-11 w-11 justify-center rounded-xl px-0", active
                ...
                <item.icon className={cn("h-5 w-5 shrink-0", active ? "text-lime-600 dark:text-lime-400" : "text-slate-500 dark:text-white/70")}/>
                {!isCollapsed && <span className="truncate">{item.title}</span>}
                {!isCollapsed && item.badge && (<span className="ml-auto rounded border border-lime-500/30 bg-lime-500/10 px-1.5 py-0.5 text-[11px] font-semibold text-lime-700 dark:border-lime-400/20 dark:bg-lime-400/10 dark:text-lime-400">
                    {item.badge}
                  </span>)}
              </Link>);
            if (isCollapsed) {
      ...
      <SidebarFooter className="border-t border-[var(--admin-border)] p-3">
        {isCollapsed ? (<Tooltip>
            <TooltipTrigger>
              <button type="button" onClick={handleLogout} className="mx-auto flex h-11 w-11 cursor-pointer items-center justify-center rounded-xl text-slate-500 hover:bg-red-50 hover:text-red-600 dark:text-white/70 dark:hover:bg-red-500/10 dark:hover:text-red-400">
                <LogOut className="h-5 w-5 shrink-0"/>
              </button>
            </TooltipTrigger>
            <TooltipContent side="right">Logout</TooltipContent>
          </Tooltip>) : (<button type="button" onClick={handleLogout} className="flex w-full cursor-pointer items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-slate-600 hover:bg-red-50 hover:text-red-600 dark:text-white/80 dark:hover:bg-red-500/10 dark:hover:text-red-400">
