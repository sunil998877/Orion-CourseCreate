import { ShieldCheck, BookOpen, Layers, FileSpreadsheet, Mic, Eye, Plus, Zap, } from "lucide-react";
...
export const UsersTable: React.FC<UsersTableProps> = ({ users, loading, onViewDetails, onAdjustCredits, }) => {
    return (<div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-colors duration-200 dark:border-white/10 dark:bg-white/[0.03] dark:shadow-none">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-white/5">
            ...
            return (<tr key={user.id} onClick={() => onViewDetails(user)} className="group cursor-pointer hover:bg-slate-50 dark:hover:bg-white/[0.04] transition-colors">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-slate-900 group-hover:text-lime-600 dark:text-white dark:group-hover:text-lime-400 transition-colors flex items-center gap-1.5">
                            {user.username}
                            {user.isVerified && (<span title="Verified User" className="inline-flex items-center">
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1">
                        <span className={cn("inline-block rounded-md px-2 py-0.5 text-[10px] font-bold border", planName === "Team" &&
                    "bg-purple-500/10 text-purple-600 border-purple-500/20 dark:text-purple-400", planName === "Pro" &&
                    "bg-lime-500/10 text-lime-700 border-lime-500/30 dark:text-lime-400", planName === "Free" &&
                    "bg-slate-100 text-slate-600 border-slate-200 dark:bg-white/10 dark:text-white/60 dark:border-white/10")}>
                          {planName} Plan
                        </span>

                        <div className="flex flex-wrap items-center gap-1">
                          {rechargeCount > 0 && (<span className="inline-flex items-center gap-0.5 rounded bg-lime-500/10 px-1.5 py-0.5 text-[9px] font-semibold text-lime-700 dark:text-lime-400 border border-lime-500/20">
                              <Zap className="h-2.5 w-2.5"/>
                              {rechargeCount} Top-up{rechargeCount > 1 ? "s" : ""} (₹{totalRechargedINR})
                            </span>)}
                          {planSubCount > 0 && (<span className="inline-flex items-center gap-0.5 rounded bg-purple-500/10 px-1.5 py-0.5 text-[9px] font-semibold text-purple-600 dark:text-purple-400 border border-purple-500/20">
                    ...
                    <td className="px-6 py-4">
                      ...
                      {reserved > 0 ? (<span className="text-[10px] font-semibold text-amber-600 dark:text-amber-400">
                          ({reserved} cr hold)
                    ...
                    <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center justify-end gap-2">
                        <button type="button" onClick={() => onViewDetails(user)} className="inline-flex items-center gap-1 rounded-lg border border-lime-500/30 bg-lime-500/10 px-2.5 py-1 text-xs font-semibold text-lime-700 hover:bg-lime-500/20 cursor-pointer dark:border-lime-400/30 dark:bg-lime-400/10 dark:text-lime-400" title="Open Full User Profile, Courses & Deductions">
                          <Eye className="h-3.5 w-3.5"/>
                          View Details
                        </button>
                        <button type="button" onClick={() => onAdjustCredits(user)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-100 cursor-pointer dark:border-white/10 dark:bg-white/5 dark:text-white/80 dark:hover:bg-white/10" title="Adjust Credits">
