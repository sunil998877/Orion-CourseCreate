...
export function emitCreditShortage(kind: CreditShortageKind, message?: string) {
    if (typeof window === 'undefined')
        return;
    window.dispatchEvent(new CustomEvent(CREDIT_SHORTAGE_EVENT, {
        detail: { kind, message },
    }));
}
export function classifyCreditShortage(status?: number, data?: CreditErrBody): CreditShortageKind | null {
    const { code, msg } = flattenCreditBody(data);
    if (code === 'gamma_credits_exhausted' ||
    ...
    if (code === 'openai_credits_exhausted' ||
        code === 'ai_rate_limit' ||
    ...
    if (code === 'elevenlabs_credits_exhausted' ||
