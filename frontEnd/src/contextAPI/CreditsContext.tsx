...
import { getWallet, getTransactions, rechargeWallet, subscribeToPlan as subscribeToPlanApi, } from '../services/walletService';
import CreditShortageModal from '../components/credits/CreditShortageModal';
import { CREDIT_SHORTAGE_EVENT, emitCreditShortage, type CreditShortageKind, } from '../utils/creditErrors';
type DeductResult = {
    success: true;
} | {
    success: false;
...
export const CreditsProvider: React.FC<{
    ...
    const hasEnoughCredits = useCallback((amount: number) => credits.remaining >= Math.round(amount), [credits.remaining]);
    const deductCredits = useCallback((amount: number, action: CreditAction): DeductResult => {
        if (amount <= 0)
            return { success: true };
    ...
    return (<CreditsContext.Provider value={value}>
      {children}
      {shortage && (<CreditShortageModal kind={shortage.kind} message={shortage.message} onClose={() => setShortage(null)}/>)}
