...
export const useCourseDownloads = () => ({
    downloadAudio: async (url?: string, id?: string) => { if (!url)
        return; try {
        await downloadBlob(url, `audio-${id || 'course'}.mp3`);
    }
    catch (e) {
        console.error(e);
        toast.error('Download failed. Please try again.');
    } },
    downloadPodcast: async (url?: string, id?: string) => { if (!url)
        return; try {
        await downloadBlob(url, `podcast-${id || 'course'}.mp3`);
    }
    catch (e) {
        console.error(e);
        toast.error('Download failed. Please try again.');
