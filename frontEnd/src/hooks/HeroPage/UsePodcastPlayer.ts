...
export const usePodcastPlayer = ({ podcastUrl, courseId, podcastScript = [] }: Args) => {
    ...
    const togglePodcastPlay = async () => {
        ...
        try {
            if (isPodcastPlaying)
                await podcastAudioRef.current.pause();
            else
                await podcastAudioRef.current.play();
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            if (!resp.ok)
                throw new Error('Failed to fetch podcast file');
    ...
    const getActivePodcastBubbleIndex = () => {
        if (!podcastScript.length || !podcastDuration || !isPodcastPlaying)
            return -1;
