...
import { createCourse, generateCourseDescription, refineCourseDescription, } from '../services/courseService';
export const useCourseGeneration = () => {
    ...
    const refineDescription = useCallback(async (currentDescription: string, prompt: string) => {
        const token = localStorage.getItem('token');
        if (!token)
            throw new Error('Authentication required');
