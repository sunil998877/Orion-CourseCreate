...
export const getRazorpayConfig = async (token: string): Promise<RazorpayConfig> => {
    ...
    if (!res.ok)
        throw new Error(data.message || "Failed to load Razorpay config");
    return data.data as RazorpayConfig;
};
export const createRazorpayOrder = async (token: string, payload: {
    amount: number;
    credits?: number;
    package_id?: string;
    plan_id?: string;
...
export const verifyRazorpayRecharge = async (token: string, payload: Record<string, unknown>) => {
...
export const verifyRazorpayPlan = async (token: string, payload: Record<string, unknown>) => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok)
        throw new Error(data.message || "Failed to verify Razorpay plan payment");
    return data.data;
};
