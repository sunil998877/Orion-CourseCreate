...
export const rechargeWallet = async (token: string, payload: number | RechargePayload, reference_id?: string): Promise<RechargeData> => {
...
export const createRechargeStripeSession = async (token: string, payload: StripeRechargeSessionPayload): Promise<StripeSessionResponse> => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok)
        throw new Error(data.message || 'Failed to create Stripe session for credit top-up');
