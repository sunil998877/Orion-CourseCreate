...
export const generateAllModulesDraft = async (modules: any[], token: string) => {
    ...
    if (!res.ok)
        throw new Error(data.message || 'Module generation failed');
    return data;
};
...
export const getModuleContents = async (courseId: string, moduleNumber: number, token: string) => {
    const res = await fetch(`${API_BASE}/module-contents?courseId=${encodeURIComponent(courseId)}&moduleNumber=${moduleNumber}`, { headers: { Authorization: `Bearer ${token}` } });
    const data = await res.json().catch(() => []);
    if (!res.ok)
        throw new Error(data.message || 'Failed to fetch module content');
    return data;
};
...
export const downloadModulePptx = async (courseId: string, moduleId: number, token: string) => {
    const res = await fetch(`${API_BASE}/courses/${courseId}/modules/${moduleId}/download-pptx`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) {
        const data = await res.json().catch(() => ({}));
