...
export const refineCourseDescription = async (currentDescription: string, prompt: string, token: string) => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok)
        throw new Error(data.message || 'Could not refine description');
    return data;
};
