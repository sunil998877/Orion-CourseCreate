...
export interface AdminDashboardStats {
  ...
  providerBreakdown?: Record<string, { totalCreditsCharged: number; count: number }>;
  transactionBreakdown: Record<string, { totalAmount: number; count: number }>;
  recentTransactions: Array<{
    ...
    action?: { actionKey: string; displayName: string; provider: string; creditCost: number } | null;
  }>;
}
...
export interface AdminUserItem {
  ...
  wallet?: {
    ...
    plan?: { name: string; monthlyCredits: number; priceInINR?: number } | null;
  } | null;
}
...
export interface AdminUserCourseDetail {
  ...
  transactions: Array<{
    ...
    referenceId?: string;
    createdAt: string;
    action?: { actionKey: string; displayName: string; provider: string } | null;
  }>;
}
...
export interface AdminCourseItem {
  ...
  createdAt: string;
  user?: { username: string; email: string } | null;
  modulesOverview: Array<{ title: string; status: string; gammaUrl?: string | null }>;
}
...
// 2. All Users & Wallets
export const fetchAdminUsers = async (search = "", page = 1, limit = 50): Promise<{ users: AdminUserItem[]; total: number }> => {
...
export const adjustUserCreditsApi = async (payload: {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.message || "Failed to adjust credits");
  return json;
};
...
export const fetchAdminPricingRules = async (): Promise<AdminPricingRuleItem[]> => {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.message || "Failed to fetch pricing rules");
  return json.data;
};

export const updatePricingRuleApi = async (id: string, payload: { creditCost?: number; isActive?: boolean }) => {
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.message || "Failed to update pricing rule");
  return json.data;
};

// 5. Transactions Ledger
export const fetchAdminTransactions = async (type = "", search = "", page = 1, limit = 50): Promise<{ transactions: AdminTransactionItem[]; total: number }> => {
  const params = new URLSearchParams();
...
// 6. Generated Courses
export const fetchAdminCourses = async (search = "", page = 1, limit = 50): Promise<{ courses: AdminCourseItem[]; total: number }> => {
  const params = new URLSearchParams();
  if (search) params.append("search", search);
  params.append("page", String(page));
  params.append("limit", String(limit));
...
// 8. Single User Detailed View (Courses & Credit Deductions)
...
// 9. Recharges & Plans History
export const fetchAdminRechargesAndPlans = async (
  type = "ALL",
  search = "",
  page = 1,
  limit = 50
): Promise<AdminRechargePlanHistoryResponse> => {
  const params = new URLSearchParams();
  if (type && type !== "ALL") params.append("type", type);
  if (search) params.append("search", search);
  params.append("page", String(page));
  ...
  const json = await res.json();
  if (!res.ok) throw new Error(json.message || "Failed to fetch recharge and plan history");
  return json.data;
};
