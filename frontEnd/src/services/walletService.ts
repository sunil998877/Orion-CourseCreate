...
export const getTransactions = async (token: string, params?: GetTransactionsParams): Promise<TransactionsData> => {
...
export const estimateCreditCost = async (token: string, actionKey: string): Promise<EstimateData> => {
    ...
    const data = await res.json().catch(() => ({}));
    if (!res.ok)
        throw new Error(data.message || 'Failed to estimate credit cost');
