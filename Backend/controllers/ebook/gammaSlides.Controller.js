...
import { buildGammaOutlineFromCourseModule, buildGammaOutlineFromPayload } from './gammaOutline.js';
...
export const generateModuleSlidesGamma = async (req, res) => {
    ...
    try {
        let outline = buildGammaOutlineFromPayload({ moduleNumber, moduleContent, slideContent });
        if (!outline.trim() && courseId) {
            ...
            outline = buildGammaOutlineFromCourseModule(mod, moduleNumber);
        }
        if (!outline.trim()) {
    ...
    try {
        ...
        if (failedModuleError || successfulResults.length < modules.length) {
            ...
            if (!PROPORTIONAL_BILLING_ENABLED) {
                ...
                const isWalletShort = failedModuleError instanceof InsufficientCreditsError ||
