...
import {
    formatTransaction,
    providerSpendPipeline,
    ledgerProviderBreakdown,
...
const loadAnalytics = async () => {
    ...
    const txByType = {};
    for (const t of txAgg) txByType[t._id] = t;

    const providerBreakdown = mergeProviderBreakdown(
        ledgerProviderBreakdown(providerAgg),
        estimatedProviders
    );

    const totalCreditsIssued =
        (txByType.RECHARGE?.totalAmount || 0) +
...
export const getAdminAnalytics = async (req, res) => {
    try {
        const data = await withCache("analytics", 20_000, loadAnalytics);
