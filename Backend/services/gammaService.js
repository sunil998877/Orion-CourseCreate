...
function mapGammaFailure(message, httpStatus) {
    ...
    if (normalized.includes('insufficient credits') || normalized.includes('billing')) {
        return new GammaApiError('Gamma slide credits are exhausted. Upgrade your Gamma plan or contact support.', 503, 'gamma_credits_exhausted');
    }
    if (httpStatus === 401 || httpStatus === 403) {
        return new GammaApiError('Gamma API authentication failed. Check GAMMA_API_KEY configuration.', 503, 'gamma_auth_error');
    }
    if (httpStatus === 429) {
        return new GammaApiError('Gamma API rate limit reached. Please try again later.', 503, 'gamma_rate_limit');
    }
...
- Generate exactly 10 slides.`;
        ...
        while (true) {
            const statusRes = await fetch(`${GAMMA_BASE}/v1.0/generations/${generationId}`, {
            ...
            if (statusData.status === 'completed') {
                const gammaLink = statusData.gammaUrl ||
                    statusData.url ||
