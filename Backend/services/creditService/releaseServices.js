    ...
    static async release(userId, reservation) {
        ...
        try {
            await session.withTransaction(async () => {
                ...
                if (reservation._id) {
                    await CreditTransaction.findByIdAndUpdate(reservation._id, { status: "RELEASED" }, { session });
                }
                else if (reservation.referenceId) {
                    await CreditTransaction.updateMany({ referenceId: reservation.referenceId, type: "RESERVE", status: "PENDING" }, { status: "RELEASED" }, { session });
                }
