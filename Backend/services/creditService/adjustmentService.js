...
/**
 * Executes a manual support adjustment to credit or debit a user's wallet
 * with a mandatory audit trail of who approved it and why.
 *
 * @param {Object} params
 * @param {string} params.userId - Target user ID
 * @param {number} params.amount - Integer amount (positive for credit, negative for debit)
 * @param {string} params.approvedBy - Name / Email / ID of the support or admin agent
 * @param {string} params.reason - Explanation or ticket reference for the adjustment
 * @param {string} [params.notes] - Optional additional notes
 * @param {string} [params.referenceId] - Optional ticket or tracking ID
 * @returns {Promise<Object>}
 */
export const processSupportAdjustment = async ({
    userId,
    amount,
    approvedBy,
    reason,
    ...
    try {
        await session.withTransaction(async () => {
            ...
            if (newBalance < 0) {
                throw new Error(
                    `Cannot debit ${Math.abs(intAmount)} credits. User only has ${balanceBefore} credits available.`
                );
            }
